    ...
    Edge: {
      ...
      properties: {
        ...
        revokedBy: {
          ...
          minLength: 1,
          description:
            'operationId of the (single) API operation that revokes (destroys) an instance of this edge — the symmetric inverse of `establishedBy`. By convention this operation is `DELETE` on the same path as the establisher (which is `PUT`), keyed by the same identifier tuple. Required: every encoded edge must be revocable so the planner can scope cleanup and so #224-style symmetry is enforceable as an L3 invariant.',
        },
        observableVia: {
          type: 'string',
          minLength: 1,
