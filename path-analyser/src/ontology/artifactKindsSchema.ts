    ...
    OperationArtifactRule: {
      ...
      properties: {
        ...
        role: {
          ...
          minLength: 1,
          description:
            'Optional ontological role this operation plays in the API surface. Used by the planner and Playwright emitter to discriminate special-case behaviour (e.g. multipart deployment routing) against the ABox instead of against a hard-coded operationId. Conventional values: `deploymentGateway` — the multipart deploy operation whose response surfaces deployed artifact identifiers (Lift 9 / #225: retired ~20 hard-coded `=== "createDeployment"` literals across `path-analyser/src/{scenarioGenerator,index,codegen/playwright/emitter}.ts`); `jobActivator` — the operation that activates (polls for / leases) jobs produced by service tasks in a deployed BPMN process (Lift 14 / #254: retired six hard-coded `=== "activateJobs"` literals across `path-analyser/src/{index,scenarioGenerator,featureCoverageGenerator}.ts`, covering search-like negative-empty coverage, service-task wiring in the planner BPMN model-spec draft, and non-existent-job-type overrides on empty-negative scenarios). When the role is consumed only by the planner (no per-step emitter dispatch) set `plannerOnly: true` so the materializer does not require a `configs/<config>/codegen/playwright/roles/<role>/` bundle.',
        },
        plannerOnly: {
          type: 'boolean',
          description:
            'Optional flag indicating that the declared `role` is consumed only by the planner (e.g. for chain-shape reasoning, model-spec drafting, or negative-empty coverage gating) and not by the per-step Playwright emitter dispatch. When `true`, the materializer treats the role as informational: it does not require a `configs/<config>/codegen/playwright/roles/<role>/` bundle and does not route matching steps through a custom call-site template. When `false` or omitted, the role is emitter-dispatched — the bundle must exist and steps matching the role render through the bundle (Lift 12 / #231 contract). Introduced by Lift 14 / #254 for the `jobActivator` role, which has no custom Playwright call-site (the generic inline path is correct for `activateJobs`).',
        },
        rules: {
          type: 'array',
          minItems: 1,
