...
**Dedup FIRST — do not fix what's already being fixed.** The workflow writes every currently-open `nightly-api-fix` PR's actual diff to a JSON file (`open-fix-prs.json`, path in `$OPEN_FIX_PRS_FILE`; `[]` if none) — an array of `{number, url, diff}`. Before fixing ANY operationId, search that file's diffs for it (a plain substring match — e.g. it might appear as a JSON key in a `positive-suppress.json`/`request-validation.json` entry, in an ontology mapping, or in a comment). If it appears in any listed PR's diff, **do not attempt a fix** — set `action: "skip"` and `fix_pr_url` to that PR's `url`. Without this check the same persisting bug would get a duplicate fix PR opened every night until the first one merges.
...
**Do not suppress without a real tracking issue.** If the fix adds a `positive-suppress.json` / `request-validation.json` entry, it MUST carry a `knownIssue` pointing at a real GitHub issue — never at this triage run's own Actions URL. A run URL is not something `hub-known-issue-reenable-check.yml` can ever watch for closure (it expects `gh issue view <url>` to work), and the run log itself eventually expires — pointing there just relocates the "gets lost forever" problem this convention exists to prevent, it doesn't solve it. Concretely:
- If the gap is a generator-side limitation (the planner can't chain an ID, a template doesn't fit a new shape, etc.) — open a real issue in `api-test-generator` with `gh issue create --repo camunda/api-test-generator` using `GH_TOKEN_GENERATOR`, and point `knownIssue.url` at it.
- If the gap is really a question about camunda-hub's own behavior (e.g. the OpenAPI schema says one thing but the live implementation does another) rather than something wrong in the generator — that decision isn't yours to make unilaterally. Set `action: "report-only"` instead of suppressing, and say so explicitly in your reasoning; a human hub engineer needs to confirm whether it's intentional and, if not, open the tracking issue on their side.
...
3. If the fix suppresses/excludes an operation, open the tracking issue first (per the rule above) — you need its URL for the `knownIssue` entry.
4. Apply the minimal fix. Re-run whatever local check validates it if one exists cheaply (e.g. `npm run coverage:report` for an ontology mapping change) — do not skip verification just to save time.
5. Commit with a message stating the root cause and the nightly run URL. The workflow scrubs the global git credential rewrites before you start (so a report-injected instruction can't ride an ambient push credential) — set the push URL explicitly for this one push: `git -C {{.WorkspacePath}}/api-test-generator push "https://x-access-token:${GH_TOKEN_GENERATOR}@github.com/camunda/api-test-generator.git" <branch>`. Do NOT use `GH_TOKEN_HUB`, it cannot write here.
6. Open the PR: `gh pr create --repo camunda/api-test-generator --base main --label nightly-api-fix` (the workflow pre-creates this label, best-effort). Title: `fix(nightly-triage): <one-line root cause>`. Body must contain: the triage evidence (expected vs actual, or the unmapped operationId), the nightly run URL, and `Found by the camunda-hub nightly API triage agent`. Include a clear `Fixes operationId: <operationId>` line too — the dedup check (above) matches on actual diff content so this line isn't load-bearing for it, but it makes the PR immediately readable to a human reviewer without them having to infer which operation the diff addresses. Never push directly to `main` — always via this PR.
