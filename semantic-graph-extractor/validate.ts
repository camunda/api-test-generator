...
async function validateExtraction() {
  ...
  try {
    ...
    const expectedTypes = ['ProcessInstanceKey', 'ProcessDefinitionKey', 'JobKey', 'UserTaskKey'];
    ...
    const createInstanceDeps = graph.edges.filter(
      (e: DependencyEdge) =>
        e.sourceOperationId === 'createProcessInstance' && e.semanticType === 'ProcessInstanceKey',
    );

    console.log(`   Found ${createInstanceDeps.length} dependencies from createProcessInstance`);
    if (createInstanceDeps.length > 0) {
      console.log(
        `   Sample: ${createInstanceDeps[0].targetOperationId} consumes ProcessInstanceKey`,
      );
    }

    // Find dependencies where activateJobs produces JobKey
    const activateJobDeps = graph.edges.filter(
      (e: DependencyEdge) => e.sourceOperationId === 'activateJobs' && e.semanticType === 'JobKey',
    );
    ...
    if (createProcessOp) {
      ...
      const semanticParams = createProcessOp.parameters.filter(
        (p: OperationParameter) => p.semanticType,
      );
