import type { CrossContaminationMap, SemanticType, SemanticTypeLibrary } from './types';
...
export class CrossContaminationAnalyzer {
  ...
  findContaminationOpportunities(
    ...
    for (const [typeName, semanticType] of Array.from(semanticTypes)) {
      const contaminants = this.findContaminantsForType(
        semanticType,
        typeGroups,
        semanticTypeLibrary,
      );
    ...
    console.log(
      `Found contamination opportunities for ${Object.keys(contaminationMap).length} semantic types`,
    );
   ...
   */
  analyzeContaminationSeverity(
    contaminationMap: CrossContaminationMap,
  ): ContaminationSeverityAnalysis {
   ...
   */
  private classifyContaminationRisk(
    targetType: string,
    contaminants: string[],
  ): 'high' | 'medium' | 'low' {
