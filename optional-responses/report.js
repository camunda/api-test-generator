...
function main() {
  const repoRoot = process.cwd().endsWith('optional-responses')
    ...
    : process.cwd();
  // Spec resolution:
  //   1. OPENAPI_SPEC_PATH env var wins unconditionally. This lets the
  //      script run outside the repo (or against an arbitrary bundled
  //      spec) without needing a configs.json next to it.
  //   2. Otherwise we look up CONFIG against configs.json (allowlist +
  //      safe-name regex) and resolve spec/<config>/bundled/...
  let specPath = process.env.OPENAPI_SPEC_PATH;
  if (!specPath) {
    ...
    specPath = path.resolve(repoRoot, 'spec', config, 'bundled', 'rest-api.bundle.json');
  }
