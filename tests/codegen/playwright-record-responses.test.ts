...
// Class-scoped guard for the `recordResponses` codegen option, now sourced
// from `configs/<config>/codegen/playwright/config.json#recordResponses`.
// The option gates two related emissions in lockstep — the import statement
...
function ctxBase() {
  return {
    ...
    configName: 'test',
    resolveConfigPath: (rel: string) => path.resolve('/unused', rel),
  };
}
...
describe('emitter: recordResponses option gates recorder import + per-step call', () => {
  test('recordResponses=true emits both the import and the per-step recordResponse block', async () => {
    const [file] = await PlaywrightEmitter.emit(COLLECTION_WITH_STEP, {
      ...ctxBase(),
      emitterConfig: { recordResponses: true },
    });
  ...
  test('recordResponses=false drops both the import and the per-step recordResponse block', async () => {
    const [file] = await PlaywrightEmitter.emit(COLLECTION_WITH_STEP, {
      ...ctxBase(),
      emitterConfig: { recordResponses: false },
    });
  ...
  test('recordResponses omitted defaults to off (matches camunda-oca config)', async () => {
    // The SDK contract is that `emitterConfig` defaults to `{}` and each
    // emitter applies its own per-knob default. PlaywrightEmitter defaults
    // recordResponses to `false`, matching the shipped camunda-oca config.
    const [withDefault] = await PlaywrightEmitter.emit(COLLECTION_WITH_STEP, {
      ...ctxBase(),
      emitterConfig: {},
    });
    const [withExplicitFalse] = await PlaywrightEmitter.emit(COLLECTION_WITH_STEP, {
      ...ctxBase(),
      emitterConfig: { recordResponses: false },
    });
    expect(withDefault.content).toBe(withExplicitFalse.content);
    expect(withDefault.content).not.toContain('recordResponse');
  });
